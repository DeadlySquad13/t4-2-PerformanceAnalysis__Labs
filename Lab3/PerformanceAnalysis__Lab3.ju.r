...
# %%
get_probability_of <-function(States, state) {
...
get_probabilities_to_stay <- function(States) {
    return(lapply(1:length(P), function(state) get_probability_of(States, state)))
}
...
env %>%
  ...
  add_generator("programmers", programmers, function() 10)
